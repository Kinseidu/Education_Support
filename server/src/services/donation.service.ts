...
const buildReference = () => uuid();

export const initiateDonation = async (input: DonationInitiateInput) => {
  ...
  if ((input.provider ?? "paystack") === "paystack") {
    ...
    donation.gatewayResponse = response.data.data;
    await donation.save();
...
export const verifyDonation = async (input: DonationVerifyInput) => {
  ...
  if (input.provider === "paystack") {
    ...
    const response = await axios.get(
      `${env.paystackBaseUrl}/transaction/verify/${input.reference}`,
      {
        headers: {
          Authorization: `Bearer ${env.paystackKey}`
        }
